...
Strophe.addConnectionPlugin('roster', {
    ...
    statusChanged: function (status) {
        if (status === Strophe.Status.CONNECTED) {
            ...
            this.connection.addHandler(this._onReceiveRosterIQ.bind(this),
                                       Strophe.NS.ROSTER, "iq", "set");
            this.connection.addHandler(this._onReceivePresence.bind(this),
                                       null, "presence");
            ...
            // notify user code
			this._doRosterChanged();
        }
    },

	// User overrideable function that recieves a reference to the roster that changed
    onChanged: function (roster)
    {
        return;
    },
	
	_doRosterChanged : function()
	{
    ...
    _onReceiveRosterIQ: function (iq) {
        ...
        } else if ((subscription === "none") || (!this.contacts[jid])) {
	...
	// User overrideable function that recieves the jid that requested a subscription
	// return true if request is acceptable, false if not.
	onSubscriptionRequest : function(jid)
	{
		// default: accept all requests
		return true;
	},

	onUnsubscribed : function(jid)
	{
		return true;
	},
    ...
    _onReceivePresence: function (presence) {
		...
		if (ptype === "subscribe") {
			...
			if (this.onSubscriptionRequest(jid))
			{
				this.connection.send($pres({to: jid, type: "subscribed"}));
				// Contact not on roster?
